...
void Hooks::Install ()
{
	...
	for ( int i = 0; i < sizeof(hooks) / sizeof(HookEntry); i++ )
	{
		*hooks[i].m_pHookPos = 0xE9;
		*(dword *)(hooks[i].m_pHookPos + 1) = (byte *)hooks[i].m_pTarget - (hooks[i].m_pHookPos + 5);
	}
	...
	*Address::Inst.CTrackManiaEditor__ConnectDirFromRoadBlock_hookpos = 0xEB;
	...
	//*(word *)Address::Inst.CTrackManiaEditor__ProcessCursor_hookpos1 = 0x9090;
	//*(word *)Address::Inst.CTrackManiaEditor__ProcessCursor_hookpos2 = 0x9090;
	//*(byte *)Address::Inst.CTrackManiaEditor__ProcessCursor_hookpos3 = 0xEB;
...
_declspec(naked) void Hooks::hook_ValidateBlockPositionAndPillars ()
{
	_asm
	{
		mov eax, 1
...
_declspec(naked) void Hooks::hook_ClearAreaForBlock ()
{
	_asm
	{
		push eax
...
bool Hooks::handle_ClearAreaForBlock ( CGameCtnBlock* pBlock )
{
	return pBlock->GetBlockInfo ()->GetType () == CGameCtnBlockInfo::BLOCKTYPE_CLIP;
}

_declspec(naked) void Hooks::hook_RemoveBlock ()
{
	_asm
	{
		push edi
...
_declspec(naked) void Hooks::hook_RemoveBlockSafe ()
{
	_asm
	{
		push edi
...
bool Hooks::handle_RemoveBlock ( CGameCtnEditor* pEditor, CGameCtnBlock* pRemovedBlock )
{
...
_declspec(naked) void Hooks::hook_IsBlockOnGround ()
{
	_asm
	{
		push ecx
...
int Hooks::handle_IsBlockOnGround ( CGameCtnBlockInfo* pBlockInfo )
{
	if ( !pBlockInfo )
		return 0;
...
_declspec(naked) void Hooks::hook_GetBlockUnitInfo ()
{
	_asm
	{
		push dword ptr [esp+8]
...
void* Hooks::handle_GetBlockUnitInfo ( CGameCtnBlockInfo* pBlockInfo, int index, bool bGround )
{
...
_declspec(naked) void Hooks::hook_CreateBlockClip ()
{
	_asm
	{
		push ecx
...
bool Hooks::handle_CreateBlockClip ( CGameCtnChallenge* pChallenge, Vec3Di pos )
{
	return pChallenge->IsPosInBounds ( pos );
}

_declspec(naked) void Hooks::hook_GetZoneAtPos ()
{
	_asm
	{
		test eax, eax
...
CGameCtnZone* Hooks::handle_GetZoneAtPos ( CGameCtnChallenge* pChallenge, Vec3Di pos )
{
	for ( int i = 0; i < pChallenge->NumBlocks (); i++ )
	{
		CGameCtnBlock* pBlock = pChallenge->GetBlock ( i );
		...
		{
			CGameCtnZone* pZone = pChallenge->GetCollection ()->GetZoneByBlockInfo ( pBlock->GetBlockInfo () );
			if ( pZone )
				return pZone;
		}
	}
...
_declspec(naked) void Hooks::hook_CreateRoadBlock ()
{
	_asm
	{
		add esp, 0x18
...
bool Hooks::handle_CreateRoadBlock ( CGameCtnEditor* pEditor, Vec3Di pos )
{
...
_declspec(naked) void Hooks::hook_InitBuildArea ()
{
	_asm
	{
		push esi
...
void Hooks::handle_InitBuildArea ( CGameCtnChallenge* pChallenge )
{
...
_declspec(naked) void Hooks::hook_FloorCursorPos ()
{
	_asm
	{
		push ecx
...
bool Hooks::handle_FloorCursorPos ( CTrackManiaEditor* pEditor, Vec3Di* pPos )
{
	return false;
}

_declspec(naked) void Hooks::hook_CreateTerrainBlockAtPos ()
{
	_asm
	{
		mov [edi+8], edx
...
void Hooks::handle_CreateTerrainBlockAtPos ( CTrackManiaEditor* pEditor, Vec3Di* pPos )
{
